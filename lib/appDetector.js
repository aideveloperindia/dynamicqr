...
function detectApp(userAgent = '', xRequestedWith = '') {
  ...
  if (ua.includes('paisa') || ua.includes('gpay') || ua.includes('google pay') || 
      ua.includes('googlepay') || ua.includes('tez')) {
    return AppType.GOOGLE_PAY;
  }

  // Additional detection: Check for PhonePe (more patterns)
  if (ua.includes('phonepe') || ua.includes('phone-pe') || ua.includes('phone_pe')) {
    return AppType.PHONEPE;
  }

  // Additional detection: Check for Paytm (more patterns)
  if (ua.includes('paytm') || ua.includes('pay-tm')) {
